...
from utils.general import non_max_suppression, check_img_size, scale_boxes, scale_segments
from utils.segment.general import masks2segments, process_mask, process_mask_native, scale_image
...
class Detector:
    ...
    def __init__(self, conf_thres=0.45, iou_thres=0.7, weight_path="", imgsz=(640, 640)):
        self.conf_thres = conf_thres
        self.iou_thres = iou_thres
        self.imgsz = imgsz
        self.model = DetectMultiBackend(weight_path, device='cpu', data='./data/rooftop.yaml')
    ...
    def get_instance_segmentation(self, img):
        ...
        pred_masks = process_mask(proto[0], det[:, 6:], det[:, :4], img_shape[1:], upsample=True)  # HWC
        det[:, :4] = scale_boxes(img_shape[1:], det[:, :4], ori_img_shape).round()  # rescale boxes to im0 size

        composed_image = self.masks(pred_masks, [colors(x, True) for x in det[:, 5]], img[0], ori_img_shape)
