...
def place_solar_panels(rooftop_parts, orig_image, solar_panel_size=(10, 20)):
    final_panels = []
    edge_maps = []
    ...
    for rooftop_part in rooftop_parts:
        ...
        rooftop_part_info = {"orientation": part_cls, "panels": []}
        ...
        rotate_matrix = cv2.getRotationMatrix2D(
            center=center, angle=rotation_angle, scale=1
        )

        rotated_mask = cv2.warpAffine(
            src=rooftop_part["mask"], M=rotate_matrix, dsize=mask.shape
        )

        # find new centers to move masks to the origin to facilitate computation
        _, _, aux_stats, aux_centroid = cv2.connectedComponentsWithStats(
            rotated_mask, 8, cv2.CV_32S
        )
        ...
        aux_mask[0 : aux_bbox_info[3], 0 : aux_bbox_info[2]] = rotated_mask[
            aux_bbox_info[1] : aux_bbox_info[1] + aux_bbox_info[3],
            aux_bbox_info[0] : aux_bbox_info[0] + aux_bbox_info[2],
        ]

        panels = []
        for i in range(0, aux_mask.shape[0] - solar_panel_size[0], solar_panel_size[0]):
            for j in range(
                0, aux_mask.shape[1] - solar_panel_size[1], solar_panel_size[1]
            ):
                if (
                    aux_mask[i, j] == 255
                    and aux_mask[i + solar_panel_size[0], j + solar_panel_size[1]]
                ):
                    # aux_mask_for_draw = cv2.rectangle(aux_mask_for_draw, (j, i), (j + solar_panel_size[1], i + solar_panel_size[0]), (255, 0, 0), 3)
                    panels.append(
                        np.array(
                            [j, i, j + solar_panel_size[1], i + solar_panel_size[0]]
                        )
                    )
        ...
        if panels:
            ...
            # undo rotation
            undo_rotate_matrix = cv2.getRotationMatrix2D(
                center=center, angle=-rotation_angle, scale=1
            )
            panels_polygon[:, 0:2] = (
                undo_rotate_matrix
                @ (
                    np.concatenate(
                        [panels_polygon[:, 0:2], np.ones(panels.shape[0])[..., None]],
                        -1,
                    ).T
                )
            ).T
            panels_polygon[:, 2:4] = (
                ...
                @ (
                    np.concatenate(
                        [panels_polygon[:, 2:4], np.ones(panels.shape[0])[..., None]],
                        -1,
                    ).T
                )
            ).T
            panels_polygon[:, 4:6] = (
                ...
                @ (
                    np.concatenate(
                        [panels_polygon[:, 4:6], np.ones(panels.shape[0])[..., None]],
                        -1,
                    ).T
                )
            ).T
            panels_polygon[:, 6:] = (
                ...
                @ (
                    np.concatenate(
                        [panels_polygon[:, 6:], np.ones(panels.shape[0])[..., None]], -1
                    ).T
                )
            ).T
            ...
            for panel in panels_polygon:
                if (
                    mask[panel[1], panel[0]] == 255
                    and mask[panel[3], panel[2]] == 255
                    and mask[panel[5], panel[4]] == 255
                    and mask[panel[7], panel[6]] == 255
                ):
                    if not contain_obstacles(mask.shape, edges, panel):
                        ...
                        rooftop_part_info["panels"].append(
                            [
                                (panel[0], panel[1]),
                                (panel[2], panel[3]),
                                (panel[4], panel[5]),
